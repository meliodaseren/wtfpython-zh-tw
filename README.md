...
Python，是一個設計優美的解釋型高級語言，它提供了很多能讓程式設計師感到舒適的功能特性。但有時，Python 的一些輸出結果對於初學者來說似乎並不是那麽一目了然。
...
**註：**所有的示例都在 Python 3.5.2 版本的交互直譯器上測試過，如果不特別說明應該適用於所有 Python 版本。

# Usage/用法

我個人建議，最好依時間順序閱讀下面的示例，並對每個示例：
- 仔細閱讀設置例子最開始的代碼。如果您是一位經驗豐富的 Python 程式設計師，那麽大多數時候您都能成功預期到後面的結果。
- 閱讀輸出結果
  + 確認結果是否如你預期。
  + 確認你是否知道這背後的原理。
...
PS: 你也可以在命令行閱讀 WTFpython。 我們有 pypi 包 和 npm 包(支持代碼高亮)。(註: 都是英文版)
...
- 當在同一行將 `a` 和 `b` 的值設置為 `"wtf!"` 的時候，Python 直譯器會創建一個新對象，然後同時引用第二個變量 (譯：僅適用於3.7以下，詳細情況請看[這裡](https://github.com/leisurelicht/wtfpython-cn/issues/13))。如果你在不同的行上進行賦值操作，它就不會 “知道” 已經有一個 `wtf！` 對象 (因為 `"wtf!"` 不是按照上面提到的方式被隱式駐留的)。它是一種編譯器優化，特別適用於交互式環境。
...
* hash brownie 指一種含有大麻成分的蛋糕，這裡是句雙關
...
* 當調用 `id` 函數時，Python 創建了一個 `WTF` 類的對象並傳給 `id` 函數。然後 `id` 函數獲取其id值 (也就是記憶體地址)，然後丟棄該對象。該對象就被銷毀了。
* 當我們連續兩次進行這個操作時，Python 會將相同的記憶體地址分配給第二個對象。因為 (在CPython中) `id` 函數使用對象的記憶體地址作為對象的 id 值，所以兩個對象的 id 值是相同的。
* 綜上，對象的 id 值僅僅在對象的生命周期內唯一。在對象被銷毀之後，或被創建之前，其他對象可以具有相同的 id 值。
...
* [Python 語法](https://docs.python.org/3/reference/grammar.html) 中對 `for` 的定義是：
  ...
  ```
其中 `exprlist` 指分配目標。這意味著對可叠代對象中的**每一項都會執行**類似 `{exprlist} = {next_value}` 的操作。
...
* `is` 運算符檢查兩個運算對象是否引用自同一對象 (它檢查兩個運算對象是否相同)。
* `==` 運算符比較兩個運算對象的值是否相等。
...
> 當前的實現為 -5 到 256 之間的所有整數保留一個整數對象數組，當你創建了一個該範圍內的整數時，你只需要返回現有對象的引用。所以改變1的值是有可能的。我懷疑這種行為在Python中是未定義行為。 :-)
...
這裡直譯器並沒有聰明到能在執行 `y = 257` 時意識到我們已經創建了一個整數 `257`，所以它在記憶體中又新建了另一個對象。
...
* 當 a 和 b 在同一行中被設置為 `257` 時，Python 直譯器會創建一個新對象，然後同時引用第二個變量。如果你在不同的行上進行，它就不會 "知道" 已經存在一個 `257` 對象了。
* 這是一種特別為交互式環境做的編譯器優化。當你在實時直譯器中輸入兩行的時候，他們會單獨編譯，因此也會單獨進行優化。如果你在 `.py` 文件中嘗試這個例子，則不會看到相同的行為，因為文件是一次性編譯的。
...
- 在以 `r` 開頭的原始字符串中，反斜杠並沒有特殊含義.
  ...
  'wt\\"f'
  ```
- 直譯器所做的只是簡單的改變了反斜杠的行為，因此會直接放行反斜杠及後一個的字符. 這就是反斜杠在原始字符串末尾不起作用的原因.
...
* 直譯器期望 `not` 標記是 `not in` 操作符的一部分 (因為 `==` 和 `not in` 操作符具有相同的優先級)，但是它在 `not` 標記後面找不到 `in` 標記，所以會拋出 `SyntaxError` 異常.
...
+ Python 提供隱式的[字符串鏈接](https://docs.python.org/2/reference/lexical_analysis.html#string-literal-concatenation)，例如,
  ...
  wtf
  ```
+ `'''` 和 `"""` 在 Python中也是字符串定界符，Python 直譯器在先遇到三個引號的的時候會嘗試再尋找三個終止引號作為定界符，如果不存在則會導致 `SyntaxError` 異常.
...
子類關系應該是可傳遞的, 對吧? (即, 如果 `A` 是 `B` 的子類, `B` 是 `C` 的子類, 那麽 `A` _應該_ 是 `C` 的子類.)
...
* 由於 `SomeClass` 會從 `str` 自動繼承 `__hash__` 方法, 所以 `s` 對象和 `"s"` 字符串的哈希值是相同的.
...
* 總結一下, 你也可以把例子拆成
  ...
  a[b] = a, b
  ```
  並且可以通過 `a[b][0]` 與 `a` 是相同的對象來證明是循環引用
  ```py
  >>> a[b][0] is a
  True
  ```
...
一些非西方字符雖然看起來和英語字母相同, 但會被直譯器識別為不同的字母.
...
>>> value = 42 # 拉丁語 e
>>> valuе = 23 # 西裡爾語 'e', Python 2.x 的直譯器在這會拋出 `SyntaxError` 異常
>>> value
42
```
...
### > Modifying a dictionary while iterating over it/叠代字典時的修改
...
* Python不支持對字典進行叠代的同時修改它.
* 它之所以運行8次, 是因為字典會自動擴容以容納更多鍵值(我們有8次刪除記錄, 因此需要擴容). 這實際上是一個實現細節. (譯: 應該是因為字典的初始最小值是8, 擴容會導致散列表地址發生變化而中斷循環.)
* 在不同的Python實現中刪除鍵的處理方式以及調整大小的時間可能會有所不同.(譯: 就是說什麽時候擴容在不同版本中可能是不同的, 在3.6及3.7的版本中到[5](https://github.com/python/cpython/blob/v3.6.1/Objects/dictobject.c#L103-L110)就會自動擴容了. 以後也有可能再次發生變化. 這是為了避免散列衝突. 順帶一提, 後面兩次擴容會擴展為32和256. 即`8->32->256`.)
...
唷, 終於刪除了. 你可能已經猜到了在我們第一次嘗試刪除 `x` 時是什麽讓 `__del__` 免於被調用的. 那讓我們給這個例子增加點難度.
...
+ (譯: 這其實是 Python 交互直譯器的特性, 它會自動讓 `_` 保存上一個表達式輸出的值, 詳細可以看[這裡](https://www.cnblogs.com/leisurelylicht/p/diao-pi-de-kong-zhi-tai.html).)

---

### > Deleting a list item while iterating/叠代列表時刪除元素
...
* 在叠代時修改對象是一個很愚蠢的主意. 正確的做法是叠代對象的副本, `list_3[:]` 就是這麽做的.
...
* `del var_name` 只是從本地或全局命名空間中刪除了 `var_name` (這就是為什麽 `list_1` 沒有受到影響).
* `remove` 會刪除第一個匹配到的指定值, 而不是特定的索引, 如果找不到值則拋出 `ValueError` 異常.
* `pop` 則會刪除指定索引處的元素並返回它, 如果指定了無效的索引則拋出 `IndexError` 異常.

**為什麽輸出是 `[2, 4]`?**
- 列表叠代是按索引進行的, 所以當我們從 `list_2` 或 `list_4` 中刪除 `1` 時, 列表的內容就變成了 `[2, 3, 4]`. 剩余元素會依次位移, 也就是說, `2` 的索引會變為 0, `3` 會變為 1. 由於下一次叠代將獲取索引為 1 的元素 (即 `3`), 因此 `2` 將被徹底的跳過. 類似的情況會交替發生在列表中的每個元素上.
...
- Python 2.x 和 Python 3.x 直譯器在列表推導式示例中的輸出差異, 在文檔 [What’s New In Python 3.0](https://docs.python.org/3/whatsnew/3.0.html) 中可以找到相關的解釋:
...
> 形式上, 如果 a, b, c, ..., y, z 是表達式, 而 op1, op2, ..., opN 是比較運算符, 那麽除了每個表達式最多只出現一次以外 a op1 b op2 c ... y opN z 就等於 a op1 b and b op2 c and ... y opN z.
...
* 對於 2, 正確的語句是 `t = ('one',)` 或者 `t = 'one',` (缺少逗號) 否則直譯器會認為 `t` 是一個字符串, 並逐個字符對其進行叠代.
* `()` 是一個特殊的標記，表示空元組.
...
簡潔勝於覆雜（優美的代碼應當是簡潔的，不要有覆雜的內部實現）
Complex is better than complicated.
覆雜勝於淩亂（如果覆雜不可避免，那代碼間也不能有難懂的關系，要保持接口簡潔）
...
如果你能輕松向人描述你的方案，那也許會是一個好方案（方案測評標準）
...
* `this` 模塊是關於 Python 之禪的覆活節彩蛋 ([PEP 20](https://www.python.org/dev/peps/pep-0020)).
...
為什麽 `Yo()._Yo__honey` 能運行? 只有印度人理解.(譯: 這個梗可能是指印度音樂人[Yo Yo Honey Singh](https://en.wikipedia.org/wiki/Yo_Yo_Honey_Singh))

#### 💡 說明:

* [名字修飾](https://en.wikipedia.org/wiki/Name_mangling) 用於避免不同命名空間之間名稱衝突.
* 在 Python 中, 直譯器會通過給類中以 `__` (雙下劃線)開頭且結尾最多只有一個下劃線的類成員名稱加上`_NameOfTheClass` 來修飾(mangles)名稱.
* 所以, 要訪問 `__honey` 對象,我們需要加上 `_Yo` 以防止與其他類中定義的相同名稱的屬性發生衝突.
...
- 不要用 `+` 去生成過長的字符串, 在 Python 中, `str` 是不可變得, 所以在每次連接中你都要把左右兩個字符串覆制到新的字符串中. 如果你連接四個長度為10的字符串, 你需要拷貝 (10+10) + ((10+10)+10) + (((10+10)+10)+10) = 90 個字符而不是 40 個字符. 隨著字符串的數量和大小的增加, 情況會變得越發的糟糕 (就像`add_bytes_with_plus` 函數的執行時間一樣)
- 因此, 更建議使用 `.format.` 或 `%` 語法 (但是, 對於短字符串, 它們比 `+` 稍慢一點).
- 又或者, 如果你所需的內容已經以可叠代對象的形式提供了, 使用 `''.join(可叠代對象)` 要快多了.
- `add_string_with_plus` 的執行時間沒有像 `add_bytes_with_plus` 一樣出現二次增加是因為直譯器會如同上一個列子所討論的一樣優化 `+=`. 用 `s = s + "x" + "y" + "z"` 替代 `s += "xyz"` 的話, 執行時間就會二次增加了.
  ```py
  def add_string_with_plus(iters):
      s = ""
      for i in range(iters):
...
* `join()` 是一個字符串操作而不是列表操作. (第一次接觸會覺得有點違反直覺)
  ...
  **💡 說明:**
  如果 `join()` 是字符串方法 那麽它就可以處理任何可叠代的對象(列表，元組，叠代器). 如果它是列表方法, 則必須在每種類型中單獨實現. 另外, 在 `list` 對象的通用API中實現一個專用於字符串的方法沒有太大的意義.

* 看著奇怪但能正確運行的語句:
  + `[] = ()` 語句在語義上是正確的 (解包一個空的 `tuple` 並賦值給 `list`)
  + `'a'[0][0][0][0][0]` 在語義上也是正確的, 因為在 Python 中字符串同時也是[序列](https://docs.python.org/3/glossary.html#term-sequence)(可叠代對象支持使用整數索引訪問元素).
...
* Python 使用 2個字節存儲函數中的本地變量. 理論上, 這意味著函數中只能定義65536個變量. 但是，Python 內置了一個方便的解決方案，可用於存儲超過2^16個變量名. 下面的代碼演示了當定義了超過65536個局部變量時堆棧中發生的情況 (警告: 這段代碼會打印大約2^18行文本, 請做好準備!):
     ```py
     import dis
     exec("""
     def f():
